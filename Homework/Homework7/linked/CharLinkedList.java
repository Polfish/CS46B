package Homework.Homework7.linked;
...
public class CharLinkedList 
{
	...
	public void duplicate(char ch) {
		CharNode temp = find(ch);
		if (temp != null) {
			CharNode duplicateNode = new CharNode(temp.getData());
			if (temp == tail || head == tail) {
				...
				duplicateNode.setNext(temp.getNext());
				temp.setNext(duplicateNode);
			}
		}
		else {
			throw new IllegalArgumentException("No such node equal to " + ch + " found in the linked list.");
		}
	}
}
